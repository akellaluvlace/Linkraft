    ...
    server.tool('dreamroll_start', 'Starts/resumes a Dreamroll session AND advances the loop. First call inits state. Subsequent calls record the previous variation (if completed scores are passed) and return the next variation\'s genome + judge prompts. Returns a stop message when the stop flag is set.', {
        ...
        if (completed) {
            ...
            if (recordedStyle && completed.filePath && fs.existsSync(completed.filePath)) {
                try {
                    ...
                    const result = (0, judges_js_1.applyStyleAdherenceDeduction)(completed.scores, htmlContent, recordedStyle, recordedMutation);
        ...
        if (pluginRoot) {
            ...
            if (prompts.length > 0) {
                ...
                if (isMutation) {
                    blocks.push(`IMPORTANT: This variation is an experimental STYLE MUTATION (${seed.mutation}).`, 'Do NOT evaluate whether it matches the base style archetype. Evaluate whether', 'the mutation WORKS as a new aesthetic. Is it coherent? Does it communicate?', 'Is there anything genuinely novel here? Score on invention, not recognition.', '');
                }
