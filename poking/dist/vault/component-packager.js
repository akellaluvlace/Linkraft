...
exports.extractLocalImports = extractLocalImports;
 ...
 */
function resolveImportPath(importPath, baseDir, localFiles) {
    const extensions = ['', '.ts', '.tsx', '.js', '.jsx', '/index.ts', '/index.tsx', '/index.js'];
    const resolved = path.posix.resolve(baseDir, importPath);
    for (const ext of extensions) {
        const candidate = resolved + ext;
        if (candidate in localFiles)
            return candidate;
    }
    return null;
}
 ...
 */
function packageComponent(input) {
    const code = {};
    ...
    const visited = new Set([input.entryFile]);
