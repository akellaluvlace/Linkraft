 ...
 */
function loadPlanDocs(projectRoot) {
    const docs = {};
 ...
 */
function extractSection(markdown, headingSubstring) {
 ...
 */
function extractBullets(markdown, max = 10) {
    const items = [];
 ...
 */
function extractTableRows(markdown, maxDataRows = 12) {
    const lines = markdown.split('\n');
    const rows = [];
    let inTable = false;
    ...
    for (const line of lines) {
        ...
        if (isRow) {
            ...
            if (isSeparator) {
                ...
                if (dataRowsCollected >= maxDataRows)
                    break;
                rows.push(line.trim());
                dataRowsCollected++;
            }
 ...
 */
function extractLeadParagraph(markdown, maxChars = 400) {
 ...
 */
function extractCommands(markdown, maxCommands = 6) {
    const commands = [];
    const seen = new Set();
    ...
    for (const span of codeSpans) {
        ...
        const looksLikeCommand = /\s/.test(cmd) && /^(npm|pnpm|yarn|bun|npx|node|deno|tsx|vitest|jest|eslint|prettier|next|vite|supabase|prisma|drizzle|make|cargo|python|pytest|pip)\b/.test(cmd);
        if (!looksLikeCommand)
            continue;
        if (seen.has(cmd))
            continue;
