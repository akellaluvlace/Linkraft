 ...
 */
export function generateContentPack(stats: SheepStats): ContentPack {
  return {
...
function generateLinkedInPost(stats: SheepStats): string {
  return [
...
function generateTwitterThread(stats: SheepStats): string[] {
  const thread: string[] = [];
...
function pickBest(items: string[]): string {
  if (items.length === 0) return '(none this session)';
 ...
 */
export function formatContentPack(pack: ContentPack): string {
  const lines: string[] = [
