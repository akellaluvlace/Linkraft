...
export function genomeToPrompt(
  ...
  const lines: string[] = [
    ...
    '',
    '════════════════════════════════════════════════════════════════════════',
    'PRIORITY TIERS (build in this order — Tier 1 first, always)',
    '════════════════════════════════════════════════════════════════════════',
    '',
    'A clean page with 10 dimensions applied well beats a chaotic page with',
    '18 dimensions fighting each other. When in doubt, drop Tier 3 and nail Tier 1.',
    '',
    '── TIER 1: MUST BE PERFECT (check these first, if they fail nothing else matters) ──',
    '',
    `  Layout:      ${genome.layoutArchetype}`,
    `  Typography:  ${genome.typography}${typography ? ` (${typography.personality}) — ${typography.heading} ${typography.headingWeight} / ${typography.body} ${typography.bodyWeight}` : ''}`,
    `  Type scale:  ${genome.typeScale ?? '(default)'}${typeScale ? ` (ratio ${typeScale.ratio}, steps ${typeScale.steps.join('/')}px)` : ''}`,
    `  Color:       ${genome.colorPalette}${harmony?.kind === 'algorithmic' ? ` (base hue ${baseHue}°)` : ''} — palette: ${palette.join(', ') || '(see scheme rules)'}`,
    `  Density:     ${genome.density}`,
    `  Radius:      ${genome.borderRadius ?? '(default)'}${borderRadius ? ` — ${borderRadius.px}` : ''}`,
    `  Shadows:     ${genome.shadows ?? '(default)'}${shadow ? ` — ${shadow.css}` : ''}`,
    `  CTA:         ${genome.ctaStyle ?? '(default)'}${cta ? ` — ${cta.css}` : ''}`,
    '',
    '  Requirements: clean grid with no dead space, CTA above fold, clear type',
    '  hierarchy (h1 > h2 > body), WCAG AA contrast on all text, 8px spacing',
    '  grid with consistent gaps, no cramped sections.',
    '',
    '── TIER 2: SHOULD BE APPLIED (after Tier 1 is solid, never at its expense) ──',
    '',
    `  Style:       ${genome.genre}  ← the dominating visual identity`,
    `  Mood:        ${genome.mood}`,
    `  Era:         ${genome.era}`,
    `  Animation:   ${genome.animation}`,
    `  Imagery:     ${genome.imagery}`,
    '',
    '  Apply the style archetype CSS signature and mood. If applying the style',
    '  would break Tier 1 layout (e.g., a CSS effect that misaligns the grid),',
    '  dial back the style, not the layout.',
    '',
    '── TIER 3: NICE TO HAVE (only if Tier 1 and 2 are clean — skip if they break layout) ──',
    '',
    `  Mutation:    ${mutationId}${mutationSpec ? ` — ${mutationSpec.summary}` : ''}`,
    `  Copy angle:  ${copyAngleId}${copyAngleSpec ? ' — see COPY ANGLE block' : ''}`,
    `  Section var: ${sectionVariationId}`,
    `  Img treat:   ${imageTreatmentId}${imageTreatmentSpec ? ' — see IMAGE TREATMENT block' : ''}`,
    `  Constraint:  ${genome.wildcard}`,
    '',
    '  These add variety and personality. But a page with perfect Tier 1 and no',
    '  Tier 3 is a gem. A page with broken Tier 1 and all 18 dimensions applied',
    '  is a discard. Prioritize accordingly.',
    '',
    constraintBlock,
    '',
    '════════════════════════════════════════════════════════════════════════',
 ...
 */
/**
 * Serializes a genome as the HTML comment header. Only the 5 most impactful
 * dimensions are listed: style, palette, typography, layout, mutation.
 * The full genome is in state.json for analysis; the comment keeps the
 * generated file lightweight and the prompt clean.
 */
export function serializeGenomeAsComment(genome: StyleGenome, variationNumber: number): string {
  ...
  const lines = [
    '<!--',
    `  DREAMROLL #${String(variationNumber).padStart(3, '0')}`,
    `  style: ${genome.genre} | palette: ${genome.colorPalette} (${baseHue}°) | type: ${genome.typography}`,
    `  layout: ${genome.layoutArchetype} | mutation: ${mutationId}`,
    '-->',
  ];
  return lines.join('\n');
}
