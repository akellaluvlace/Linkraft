...
Overnight autonomous design generator using an 18-dimension Style Genome organized into **priority tiers**. Tier 1 (layout, typography, color, spacing) must be perfect; Tier 2 (style archetype, mood, era, animation) is applied after; Tier 3 (mutations, image treatment, copy angle, constraints) only if Tiers 1 and 2 are clean. "A clean page with 10 dimensions applied well beats a chaotic page with 18 dimensions fighting each other." Generates standalone HTML landing pages with Lucide icons (via CDN) and real Unsplash photos matched to the product brief. Three judges score desktop AND mobile (375x667) separately from HTML/CSS source code (no Playwright, no screenshots). CSS custom properties in `:root` make every variation instantly editable. Anti-convergence guardrails prevent repetition. Mutation/density compatibility checks prevent garbage combinations. Evolves toward gems. Never stops until you say so.
